...
export async function updateDiagram(id: string, name: string | undefined, projectId: string, dslText?: string, nodesData?: Record<string, {x: number, y: number}>): Promise<Diagram> {
  if (Features.mockData) {
    queryClient.setQueryData<Diagram[]>(diagramKeys.byProject(projectId), (prev = []) =>
      prev.map((d) => (d.id === id ? { ...d, name: name || d.name, dslText: dslText ?? null, nodesData, updatedAt: new Date().toISOString() } : d)),
    );
    return Promise.resolve({ id, name: name || '', projectId, description: undefined, dslText: dslText ?? null, publishedDslText: null, nodesData, versionTag: 'v1', viewport: {x:0, y:0, zoom:1}, updatedBy: null, createdAt: '', updatedAt: '' });
  }
